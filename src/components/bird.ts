...
export class Bird implements BirdType {
    ...
    private checkHitSpike = (sp:number) => {
        ...
        if (this.direction == directions.RIGHT) {
            ...
            if (
                this.checkInsideTriangle([game.bird.x + this.w/3.2, game.bird.y],[[x1,y1],[x2,y2],[x3,y3]])
                || this.checkInsideTriangle([game.bird.x + this.w/3.2, game.bird.y + this.h/3.1],[[x1,y1],[x2,y2],[x3,y3]])
            ) this.gameOver()
            ...
            y3 = sp + game.spike.h / 8 + 18
            if (
                this.checkInsideTriangle([game.bird.x, game.bird.y],[[x1,y1],[x2,y2],[x3,y3]])
                || this.checkInsideTriangle([game.bird.x, game.bird.y + this.h/3.1],[[x1,y1],[x2,y2],[x3,y3]])
            ) this.gameOver()
        }
    }
    ...
    private checkInsideTriangle = (point:number[], triangle:number[][]) => {
        ...
        const barycentric1 = ((v1[1] - v2[1]) * (point[0] - v2[0]) + (v2[0] - v1[0]) * (point[1] - v2[1])) / denom;
        const barycentric2 = ((v2[1] - v0[1]) * (point[0] - v2[0]) + (v0[0] - v2[0]) * (point[1] - v2[1])) / denom;
